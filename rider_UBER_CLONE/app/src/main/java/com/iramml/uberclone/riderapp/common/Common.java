...
public class Common {
    ...
    public static String tokensss;
    public static String driverID="";
    public static LatLng currenLocation;
    public static LatLng currenLocation1=null;

    public static double getPrice(double km, int min){
        return ConfigApp.baseFare + (ConfigApp.timeRate * min) + (ConfigApp.distanceRate * km);
    }
    public static double getPrice1(double km, int min){
        return ConfigApp.baseFare + (ConfigApp.timeRate * min) + (ConfigApp.distanceRate1 * km);
    }
    public static double getPrice2(double km, int min){
        return ConfigApp.baseFare + (ConfigApp.timeRate * min) + (ConfigApp.distanceRate2 * km);
    }
    ...
    public static void sendRequestToDriver(final String driverID, final IFCMService mService, final Context context, final LatLng lastLocation)
    {
        ...
        DatabaseReference tokenss=FirebaseDatabase.getInstance().getReference(Common.token_tbl);

        tokenss.orderByKey().equalTo(userID).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot postSnapShot:dataSnapshot.getChildren()){
                    tokensss = postSnapShot.getValue(String.class);
                }
            }
        ...
        tokens.orderByKey().equalTo(driverID).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot postSnapShot:dataSnapshot.getChildren()){
                    ...
                    //Toast.makeText(context.getApplicationContext(), token , Toast.LENGTH_SHORT).show();
                    mService.sendMessage(content).enqueue(new Callback<FCMResponse>() {
                        @Override
                        public void onResponse(Call<FCMResponse> call, Response<FCMResponse> response) {
                           // Toast.makeText(context.getApplicationContext(), String.valueOf(response.body().success) , Toast.LENGTH_SHORT).show();
                        ...
                        public void onFailure(Call<FCMResponse> call, Throwable t) {
                            ...
                            Toast.makeText(context.getApplicationContext(), "request not sent" , Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
