...
public class CallDriverActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Common.userID = Objects.requireNonNull(HomeActivity.account).getId();
            ...
            public void onClick(View v) {
               // Toast.makeText(getApplicationContext(), driverID , Toast.LENGTH_SHORT).show();
                if(driverID!=null && !driverID.isEmpty())
                    Common.sendRequestToDriver(Common.driverID, mService, getApplicationContext(), new LatLng(lastLocation.latitude, lastLocation.longitude));
            }
        });
    }
    ...
    private void loadDriverInfo(String driverID) {
        FirebaseDatabase.getInstance().getReference(Common.user_driver_tbl)
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(user.getUserphoto()!=null && !TextUtils.isEmpty(user.getUserphoto()))
                    Picasso.get().load(user.getUserphoto()).into(imgAvatar);
